...
class Model:
    ...
    def reuse(self, img0, img1, scale):
        ...
        imgf0 = F.interpolate(img0, scale, mode="bilinear", align_corners=False)
        imgf1 = F.interpolate(img1, scale, mode="bilinear", align_corners=False)

        flow01 = self.flownet(imgf0, imgf1)
        flow10 = self.flownet(imgf1, imgf0)
        
        def flo_resize(flo, _hw):
            _h, _w = _hw
            ori_flo_h, ori_flo_w = flo.shape[2], flo.shape[3]
            # flo res resize
            flo = F.interpolate(flo, size=_hw, mode='bilinear', align_corners=False)
            # flo value resize
            flo[:, 1:] = flo[:, 1:] * (_h / ori_flo_h)
            flo[:, 0:1] = flo[:, 0:1] * (_w / ori_flo_w)
            return flo

        flow01, flow10 = map(flo_resize, [flow01, flow10], [(288, 480), (288, 480)])        

        # flow01 = F.interpolate(flow01, (288,480), mode="bilinear", align_corners=False) * (288 / scale[0])
        # flow10 = F.interpolate(flow10, (288,480), mode="bilinear", align_corners=False) * (480 / scale[1])
    ...
    def inference(self, img0, img1, reuse_things, timestep=0.5):
